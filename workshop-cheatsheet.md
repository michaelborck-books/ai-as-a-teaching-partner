# AI in HR Education: One-Hour Faculty Workshop

**Focus: Simple prompts → powerful teaching outcomes**  
**Audience: HR lecturers (low-tech, smart, practical)**  
**Goal: Show potential without overwhelming**

---

## 🎯 Opening Hook (5 minutes)

**"What if your students could practice HR scenarios infinitely, with instant feedback?"**

- No coding required
- No software installation  
- Just plain English instructions
- Start small, scale gradually

---

## 💡 The Core Concept: Chat Interface as Teaching Tool (10 minutes)

### Traditional vs. AI-Enhanced

| Traditional Approach | AI-Enhanced Approach |
|-------------------|---------------------|
| Case study (read once) | Conversation simulation (practice repeatedly) |
| Group discussion (limited participation) | Individual role-play (everyone practices) |
| Written assignment (delayed feedback) | Immediate AI feedback |

### Why Chat Interface Works
- **Zero learning curve** - Everyone can use a website
- **Process over product** - Focus on thinking, not final answer
- **Engagement through interaction** - Active vs. passive learning

---

## 🚀 Live Demo: Three Quick Wins (15 minutes)

### 1. **Conversation Starter** (2 minutes)
```
You are an HR manager conducting a termination meeting. 
I am the employee being terminated. 
Start the conversation professionally but with empathy.
```

### 2. **Instant Feedback Generator** (2 minutes)  
```
Review this HR response and suggest improvements:
"[paste student response here]"
Focus on: tone, legal compliance, clarity.
```

### 3. **Scenario Creator** (2 minutes)
```
Create 3 different performance review scenarios:
- Entry-level employee
- Senior manager  
- Remote worker
Include specific challenges for each.
```

---

## 🔄 Updating Existing Activities (10 minutes)

### Activity Makeovers

**Current Activity → AI-Enhanced Version**

**Before:** "Read case study and discuss"
**After:** "Practice conversation with AI, then debrief"

**Before:** "Write 500-word reflection"  
**After:** "Get AI feedback, then improve reflection"

**Before:** "Group role-play (3 students participate)"
**After:** "Individual AI practice, then share insights"

---

## 📝 Student Prompt Library (5 minutes)

### Give Students These "Superpowers"

**For Practice:**
- "Act as [HR role] while I practice [skill]"
- "Give me feedback on my [HR scenario] approach"

**For Creativity:**  
- "Generate 3 different [HR policy] options"
- "What are pros/cons of this [HR decision]?"

**For Critical Thinking:**
- "Challenge this HR assumption: [statement]"
- "What ethical issues in this [HR situation]?"

---

## 🎯 Assessment Design Shift (5 minutes)

### From Product to Process

**Traditional Assessment:** Submit final HR plan  
**Process Assessment:** Show AI conversation + reflection

**Example:**
1. **AI Practice:** Handle difficult conversation (recorded)
2. **Self-Analysis:** Student identifies what worked/didn't  
3. **Improvement:** Practice again with refined approach
4. **Reflection:** What did you learn about yourself?

---

## 🛠️ Getting Started Today (5 minutes)

### Your First Steps
1. **Personal experiment** (15 minutes): Try one prompt
2. **Student introduction** (next class): One AI activity
3. **Small pilot** (this semester): One assignment enhancement

### No "App" Required
- ChatGPT (chat.openai.com)
- Claude (claude.ai)  
- Any AI chat interface works

---

## 📚 Your Next Steps

**This workshop is chapter 1 of a comprehensive guide:**

- **Chapter 3:** Your implementation plan
- **Chapter 4:** Six essential techniques  
- **Chapter 5:** Conversation simulations
- **Chapter 7:** Assessment redesign
- **Appendix A:** Copy-paste prompt library

**Full book:** `github.com/michaelborck-education/ai-teaching-partner-hr`

---

## 💬 Discussion & Questions (5 minutes)

**Key discussion starters:**
- Which of your current activities could benefit from AI practice?
- What concerns do you have about academic integrity?
- How might this change student engagement?

---

## 🎁 Closing Thought

**"AI doesn't replace your teaching expertise. It gives every student unlimited practice opportunities with your guidance."**

*Start small. Think process. Focus on engagement.*